...
function createTeXViewGroup(element, rootData) {
    ...
    var programmaticallySelectedId = rootData["programmaticallySelectedId"]; // Single selection
    var programmaticallySelectedIds = rootData["programmaticallySelectedIds"] || []; // Multiple selection
    ...
    rootData['data'].forEach(function (data) {
        ...
        if (single) {
            ...
            if (programmaticallySelectedIds.includes(id) || selectedIds.includes(id)) {
                item.setAttribute("style", selectedStyle);
                if (!selectedIds.includes(id)) {
                    selectedIds.push(id); // Ensure it's added to the selectedIds
                }
            }
        }
            ...
            if (clickedId === id) {
                if (single) {
                    ...
                    if (selectedIds.includes(clickedId)) {
                        // Deselect
                        clickedElement.setAttribute('style', normalStyle);
                        selectedIds.splice(selectedIds.indexOf(clickedId), 1);
                    } else {
                        // Select
                        clickedElement.setAttribute('style', selectedStyle);
                        selectedIds.push(clickedId);
                    }
                    onTapCallback(JSON.stringify(selectedIds));
                }
